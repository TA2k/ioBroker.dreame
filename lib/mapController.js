...
const { getIvForModel, decryptMapPayload } = require('./dreame');
...
class MapController {
  ...
  /**
   * Wie _writeMerged, aber auf ein festes 200-ms-Sammelfenster gedrosselt (wie HAs eigenes
   * refresh_map, siehe Kommentar an _refreshMap): der eigentliche Lastpunkt ist NICHT das
   * Rendering, sondern die State-Schreibfrequenz waehrend der Reinigung (JEDER P-Frame loest
   * hier sonst einen eigenen setState aus). Trailing-Edge: die letzte gemergte Karte innerhalb
   * des Fensters geht nie verloren, sie wird spaetestens am Fensterende geschrieben.
   * mapMerger.process() selbst laeuft unthrottled weiter, nur die sichtbare State-Ausgabe wird
   * gedrosselt.
   */
  ...
  async _holeFrischeKarte(device) {
    ...
    try {
        ...
        for (const p of out) {
          ...
          if (p.piid === 3) freshObj = p.value;
            ...
            if (values[0] === '0') {
              ...
              if (values.length === 3) {
                // HA _request_i_map macht dasselbe: object_name (values[1]) + Key-Suffix
                // (values[2]) wieder zu "<object_name>,<keySuffix>" zusammensetzen - exakt
                // das Format, das der piid-3-Pfad unten (Komma-Kuerzung + AES-Decrypt)
                // fuer dasselbe Modell bereits verifiziert verarbeitet. Kein Parallelpfad,
                // keine neue Krypto-Logik (Issue #138). Debug statt warn: old_map_data-Pushes
                // sind waehrend der Reinigung haeufig, kein Log-Spam gewuenscht. Key-Suffix
                // NUR gekuerzt loggen - Nutzer werden gebeten, genau diese Zeile aus dem Log
                // ins oeffentliche GitHub-Issue zu kopieren, der volle Wert waere dort dann
                // oeffentlich einsehbares Schluesselmaterial.
                this.log.debug(
                  `[MAP] old_map_data mit AES-Key erhalten (piid 13): flag=${values[0]} object_name=${values[1]} key_suffix=${values[2].slice(0, 4)}...(len=${values[2].length})`,
                );
                freshObj = values[1] + ',' + values[2];
              } else {
                freshObj = values[1];
              }
            }
      ...
      if (freshObj) {
        ...
        const t0 = Date.now();
        // [MAP-DIAG] TEMPORAER (Issue #124, r2253c 404-Meldungen): validateStatus:()=>true,
        // damit wir bei einem Fehlschlag den vollen Kontext loggen koennen (Race/Region/
        // object_name-Format unterscheiden), bevor der Fehler ins bestehende outer catch
        // dieser Methode geht — der Fallback auf die persistente Karte in getMap() bleibt
        // dadurch unveraendert. Bei Erfolg (< 400) KEINE zusaetzlichen Logs. Wieder entfernen
        // in 0.4.3, sobald genug Nutzer-Logs vorliegen.
        let resp = await this.requestClient({ method: 'get', url, validateStatus: () => true });
        const dt = Date.now() - t0;
        // Fix Issue #124: r2253c/r2253w und ähnliche neuere Modelle liefern
        // object_name als "<mapId>,<suffix>", aber der OSS-Storage kennt nur
        // <mapId>. Bei 404 mit Komma: retry mit gekürztem Namen.
        if (resp.status === 404 && freshObj && freshObj.includes(',')) {
          const truncatedObj = freshObj.split(',')[0];
          this.log.debug(
            `[MAP] object_name enthaelt Komma-Suffix, versuche Fallback: ${freshObj} -> ${truncatedObj}`,
          );
          try {
            const retryUrl = await this.getFile(truncatedObj, device);
            const t1 = Date.now();
            const retryResp = await this.requestClient({
              method: 'get',
              url: retryUrl,
              validateStatus: () => true,
            });
            ...
            if (retryResp.status < 400) {
              const bodyLen = retryResp.data
                ? typeof retryResp.data === 'string'
                  ? retryResp.data.length
                  : JSON.stringify(retryResp.data).length
                : 0;
              this.log.info(`[MAP] Fresh-Map geladen nach Kuerzung des object_name-Suffix (${bodyLen} B, ${dt1}ms)`);
              resp = retryResp;
              // Fix Issue #124 Layer 2 (siehe MAP_AES_ANALYSE.md): der Komma-Suffix im
              // ORIGINALEN object_name (nicht truncatedObj) ist kein OSS-Artefakt,
              // sondern das Schluesselmaterial fuer eine AES-256-CBC-verschluesselte
              // Payload. Ohne Entschluesselung faellt der spaetere zlib-Magic-Byte-Check
              // (0x78) durch ("kein zlib-Frame erkannt"), obwohl der Download an sich
              // erfolgreich war.
              const keySuffix = freshObj.split(',')[1];
              const ivForModel = getIvForModel(device.model);
              if (!ivForModel) {
                // Persistenter Merker (Issue #138): fehlt der IV-Eintrag fuer dieses Modell,
                // wird die Basiskarte grundsaetzlich nie ladbar sein — kein Timing-Problem beim
                // Adapterstart. _kartenPaketEmpfangen nutzt das, um statt des generischen
                // "Adapter neu starten"-Hinweises eine passende Meldung auszugeben.
                this.unsupportedMapModel = device.model;
                this.log.warn(
                  `[MAP-AES] Komma-Suffix erkannt aber IV unbekannt fuer Modell ${device.model} — Entschluesselung uebersprungen, User bitte melden`,
                );
              } else {
                try {
                  const decrypted = decryptMapPayload(retryResp.data, keySuffix, ivForModel);
                  const ciphertextLen =
                    typeof retryResp.data === 'string' ? retryResp.data.length : (retryResp.data || '').length;
                  this.log.info(
                    `[MAP-AES] Karte entschluesselt (${ciphertextLen} B -> ${decrypted.length} B, model=${device.model})`,
                  );
                  resp.data = decrypted;
                } catch (aesErr) {
                  this.log.warn(`[MAP-AES] Entschluesselung fehlgeschlagen: ${aesErr.message}`);
                  this.log.warn(`[MAP-AES] model=${device.model} key_suffix=${keySuffix} iv_len=${ivForModel.length}`);
                }
              }
            } else {
              this.log.warn('[MAP-DIAG-2] Retry mit gekuerztem object_name schlug fehl');
              this.log.warn(`[MAP-DIAG-2] truncated_object_name=${truncatedObj}`);
              this.log.warn(`[MAP-DIAG-2] retry_url=${retryUrl}`);
              this.log.warn(`[MAP-DIAG-2] http_status=${retryResp.status} dt_ms=${dt1}`);
              this.log.warn(`[MAP-DIAG-2] content_type=${retryResp.headers && retryResp.headers['content-type']}`);
              this.log.warn(
                `[MAP-DIAG-2] response_body_head=${
                  typeof retryResp.data === 'string'
                    ? retryResp.data.slice(0, 300)
                    : JSON.stringify(retryResp.data).slice(0, 300)
                }`,
              );
            }
          } catch (retryErr) {
            this.log.warn(`[MAP-DIAG-2] Retry-Aufruf komplett gescheitert: ${retryErr.message}`);
          }
        }
        if (resp.status >= 400) {
          this.log.warn(`[MAP-DIAG] model=${device.model} did=${device.did}`);
          this.log.warn(`[MAP-DIAG] object_name=${freshObj}`);
          this.log.warn(`[MAP-DIAG] download_url=${url}`);
          this.log.warn(`[MAP-DIAG] http_status=${resp.status} dt_ms=${dt}`);
          this.log.warn(`[MAP-DIAG] content_type=${resp.headers && resp.headers['content-type']}`);
          this.log.warn(
            `[MAP-DIAG] response_body_head=${
              typeof resp.data === 'string' ? resp.data.slice(0, 300) : JSON.stringify(resp.data).slice(0, 300)
            }`,
          );
      ...
      if (this.mapMerger.needMapRequest) {
        ...
        if (!this.mapMerger.current) {
          ...
          if (!this._noBaseMapWarn || now2 - this._noBaseMapWarn > 60000) {
            ...
            if (this.unsupportedMapModel) {
              // Issue #138: kein Timing-Problem, sondern der fehlende AES-IV-Eintrag fuer
              // dieses Modell (s. _holeFrischeKarte) — der generische Neustart-Hinweis waere
              // hier irrefuehrend, da ein Neustart das Problem nicht loest.
              this.log.error(
                `Kartenformat für dieses Modell (${this.unsupportedMapModel}) wird aktuell nicht unterstützt – bitte im GitHub-Issue melden.`,
              );
            } else {
              this.log.error(
