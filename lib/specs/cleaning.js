...
module.exports = {
  statusStates: [
    ...
    // SIID 4 PIID 52 (mop-in-station) is defined here so the runtime REMOVE override in
    // lib/specs/model-overrides/all-vacuum.js can access its meta entry. The property is
    // unusable in practice (permanently 0, never updates on r6001*, r9419*, r2253*), so
    // all-vacuum removes it from every device's object tree at spec load time, and
    // Adapter#_cleanupDeadMopInStation deletes any pre-existing objects on first run
    // after upgrade. See issue #119.
    //
    // SIID 4 PIID 53 is NOT a persistent installed-state: emits short ~1s 0->1->0 pulses
    // during mechanical mop handling only. Renamed via nameKey; the state id stays
    // 'mop-pad-installed' so existing user scripts on the old path keep working. The real
    // pad-presence signal lives on SIID 4 PIID 6 for the families that expose it (see
    // model-overrides/r6001.js and r9419.js, REMAP 4-6).
    { id: 'mop-in-station',          siid: 4, piid: 52, nameKey: 'vacuum.status.mop-in-station',          type: 'number', role: 'value' },
    { id: 'mop-pad-installed',       siid: 4, piid: 53, nameKey: 'vacuum.status.mop-handling-pulse',      type: 'number', role: 'value' },
