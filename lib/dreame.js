...
// [MAP-AES] Modellabhaengige AES-IVs fuer verschluesselte Kartendaten (object_name-
// Komma-Suffix, siehe MAP_AES_ANALYSE.md Issue #124). Bewusst nur Modelle aufnehmen,
// die konkret verifiziert sind (Block B/E dort) - NICHT voreilig die volle
// HA-Referenztabelle uebernehmen, deren Modellbindung nicht restlos geklärt ist.
const MODEL_IV_TABLE = [
  { prefix: 'dreame.vacuum.r2253', iv: 'NRwnBj5FsNPgBNbT' }, // r2253c, r2253w u.a. (Issue #124)
];

// Liefert den festen AES-IV fuer ein Geraetemodell (Prefix-Match, case-insensitive),
// oder undefined, wenn das Modell nicht in MODEL_IV_TABLE steht.
function getIvForModel(model) {
  if (!model) return undefined;
  const m = String(model).toLowerCase();
  const entry = MODEL_IV_TABLE.find((e) => m.startsWith(e.prefix));
  return entry ? entry.iv : undefined;
}

// Entschluesselt eine AES-256-CBC-verschluesselte Kartenpayload (siehe
// MAP_AES_ANALYSE.md Block B/D). ciphertext darf ein Buffer oder ein (ggf.
// URL-sicherer) Base64-String sein. keySuffix ist der Teil aus object_name nach
// dem ersten Komma, iv der modellabhaengige IV-String aus MODEL_IV_TABLE. Kein
// PKCS7-Unpadding - zlib toleriert Padding-Reste am Ende (wie im HA-Original).
function decryptMapPayload(ciphertext, keySuffix, iv) {
  const cipherBuf = Buffer.isBuffer(ciphertext)
    ? ciphertext
    : Buffer.from(String(ciphertext).replace(/-/g, '+').replace(/_/g, '/'), 'base64');
  const keyBuf = Buffer.from(crypto.createHash('sha256').update(String(keySuffix)).digest('hex').slice(0, 32), 'utf8');
...
const decodeMultiMapData = (mapDataStr, angle, model) => {
  const _cryptoJs = require('crypto-js');
  const _buffer = require('node:buffer');
  ...
  this.getMultiMap = function (angle) {
    ...
    multiMap.robotPos = _this.rotateMapPosWithAngle(_this.multiMap.robotPos, rotateAngle);
  ...
  if (aesKey) {
    ...
    // [MAP-AES] IV ist modellabhaengig (MAP_AES_ANALYSE.md Block B), nicht pauschal
    // leer. Fuer Modelle ohne Eintrag in MODEL_IV_TABLE bleibt der historische leere
    // IV als Fallback bestehen - der bestand bereits vor diesem Fix und soll kein
    // bislang unbeanstandetes Modell brechen (siehe Block E dort).
    const ivForModel = getIvForModel(model);
    if (!ivForModel) {
      console.debug(`[MAP-AES] kein IV fuer Modell ${model} bekannt, benutze leeren IV (Legacy)`);
    }
    const iv = _cryptoJs.default.enc.Utf8.parse(ivForModel || '');
